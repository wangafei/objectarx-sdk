...
// 3. Note that although utf8Ptr() is a const member function, it may reallocate the 
//    internal buffer in AcString
//
...
public:
   ...
   ACBASE_PORT  AcString & assign(const char *psz, Encoding encoding);
    ...
    /// <description>
    /// Compare the string to another AcString object
    /// </description>
    ...
    /// </returns>
    ACBASE_PORT int  collate(const AcString & acs) const;
    ...
    /// Zero if the strings are identical, <0 if this AcString object is less than acs, or >0 if this AcString object is greater than acs.
    /// </returns>
    ACBASE_PORT int collateNoCase(const AcString& acs) const;
    ...
    /// <returns>
    /// Retrun the number of replaced instances of the substring. Zero if the string is not changed.
    /// </returns>
    ...
    /// <returns>
    /// Retrun the number of replaced instances of the wchOld. Zero if the string is not changed.
    /// </returns>
    ACBASE_PORT int replace(const wchar_t wchOld, const wchar_t wchNew);
    ...
    /// <description>
    /// Set the character at the givin postion to the specified character.
    /// </description>
    ...
    /// <description>
    /// Get one character at the givin postion from the string.
    /// </description>
    /// <param name="nIndex">Zero-based postion of character in the string.</param>
    /// <returns>
    /// Return the character at the specified position in the string
    /// </returns>
    ACBASE_PORT wchar_t getAt(int nIndex) const;
    ...
    /// This value does not include space for a null terminator.</param>
    /// <returns>
    /// wchar_t pointer to the object’s (null-terminated) character buffer.
    /// </returns>
    ACBASE_PORT ACHAR* getBuffer(int nMinBufferLength);

    /// <description>
    /// Use releaseBuffer() to end use of a buffer allocated by getBuffer function. The address returned by getBuffer
    /// is invalid after the call to releaseBuffer.
    /// </description>
    /// <param name="nMinBufferLength">The new length of the string in characters, not counting a null terminator.
    /// If the string is null-terminated, the -1 default value sets the string size to the current length of the string.
