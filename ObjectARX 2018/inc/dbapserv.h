...
public:
    ...
    //
    ACDBCORE2D_PORT virtual Adesk::Boolean isURL(const ACHAR * pszURL) const;
    ...
    /// <param name="strUrl">return the corresponding URL, if any</param>
    /// <returns>Return true if this is a remote file, false otherwise.</returns>
    ACDBCORE2D_PORT
    Adesk::Boolean isRemoteFile(const ACHAR * pszLocalFile, AcString& strUrl);
    ...
    /// <param name="strLocalFile">return local file name </param>
    /// <returns>Returns Acad::eOk if successful.</returns>
    ACDBCORE2D_PORT
    Acad::ErrorStatus getRemoteFile(const ACHAR * pszURL, AcString& strLocalFile);     // bIgnoreCache = false
    ...
    ACDBCORE2D_PORT virtual Adesk::Boolean isCloudFile(const ACHAR* pszCloudFile) const;
