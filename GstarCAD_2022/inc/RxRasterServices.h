...
public:
  ...
  */
  ...
  virtual OdGiRasterImagePtr loadRasterImage(BYTE* buffer, DWORD size) = 0;
  ...
  */
  ...
  virtual bool isSupportDecoding(const wchar_t *imgPath) const = 0;

  // 以下2个函数考虑跨平台，需要重新设计

  virtual OdGiRasterImagePtr createFromHBITMAP(HBITMAP hbmp, HPALETTE hpall = 0, bool bTransparency = false) = 0;

  virtual HBITMAP drawToDIBSection(OdGiRasterImagePtr pRaster, int w, int h, int bpp) = 0;
};
