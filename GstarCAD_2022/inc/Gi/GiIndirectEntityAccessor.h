...
class ODGI_EXPORT OdGiIndirectEntityDataForBlockRef : public OdGiIndirectEntityDataBase
{
  ...
  public:
    ...
    virtual OdGeMatrix3d blockTransform(const OdGiDrawable *pDrw, OdGiViewportDraw* pVd) const = 0;
    ...
    virtual OdGiClipBoundary *getClipBoundary(OdGiBaseVectorizer*pBaseVectorizer,const OdGiDrawable *pDrw, bool& enabled, bool plotting = false, OdGiAbstractClipBoundary** pClipInfo = NULL) const = 0;

    // add by llg  用于判断注释性块是否需要绘制
    virtual bool isNeedDraw(OdGiBaseVectorizer*pBaseVectorizer, const OdGiDrawable *pDrw) const = 0;
};
