...
public:
  ...
  bool isContainLoop(OdGeHatchLoop* pLoop, bool bTryOpt); // LOOP 完全包含检测 -- 多段线求面积使用
  const OdGeExtents3d& boundBlock() const;
  bool isDisjoint(OdGeHatchLoop* pLoop, const OdGeTol& tol = OdGeContext::gTol);
...
public:
  ...
  bool getCountClockWiseLoops(std::vector<OdGeVoidPointerArray> &boundary,
    std::vector<OdGeVoidPointerArray> &ids,
    std::vector<GeHB::LOOP_TYPE> &boudaryTypes,
    std::vector<bool> &closedFlag);
