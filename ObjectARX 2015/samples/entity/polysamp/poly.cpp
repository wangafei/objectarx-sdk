                                         ...
                                         AcDbIntPtrArray&       gsMarkers) const
{
    ...
    INT_PTR nIndex = subEntId.index();
    INT_PTR nIndexPlusOne = subEntId.index()+ 1;
    switch (subEntId.type()) {

    case AcDb::kVertexSubentType:
        
        if (nIndex == 0) 
            gsMarkers.append(nIndexPlusOne);
        else
            gsMarkers.append(nIndex);
        break;
...
AsdkPoly::subSubentPtr(const AcDbFullSubentPath& path) const
{
    ...
    case AcDb::kVertexSubentType:
        ...
        pt = new AcDbPoint();
        // 64Bit: The return type of "Adesk::GsMarker" (INT_PTR) may 
        // contain pointer values, but for this specific case, it 
        // should remain in the range of "int" even though it's of type 
        // "INT_PTR". But we can never be sure if we're gonna need more 
        // than 2 billion vertices in the future, if that need arises 
        // we'll need to modify "AcArray::operator[]" to take INT_PTR.
        pt->setPosition(vertexArray[AcIntPtrToInt(subEntId.index(), INT_MAX)]);
        pt->setNormal(normal());
    ...
    case AcDb::kEdgeSubentType:
        ...
        // than 2 billion vertices in the future, if that need arises 
        // we'll need to modify "AcArray::operator[]" to take INT_PTR.
        line->setStartPoint(vertexArray[AcIntPtrToInt(subEntId.index(), INT_MAX)]);
        line->setEndPoint(vertexArray[AcIntPtrToInt(subEntId.index(), INT_MAX) + 1]);
