...
public:
  ...
  static GCBASE_PORT unsigned wcharToAnsiOrUtf8(wchar_t wch, char *pDestBuf,
    unsigned nDestBufSize,
    bool bToAnsi,
    bool bUseCIF, bool bExpandLF);

  static GCBASE_PORT unsigned wcharToUtf16(wchar_t wch,
    char * pDestBuf,
    unsigned nDestBufSize,
    bool bLittleEndian,
    bool bExpandLF);
  ...
  template <class ChType> GSOFT_DEPRECATED static bool putMIF(wchar_t wch,
  ...
  static GCBASE_PORT int isNativeToCodePage(wchar_t wch,
    unsigned nCodePage,
    char *pChNative,
    int bufferLen);

  static GCBASE_PORT int isNativeToCodePageId(wchar_t wch,
