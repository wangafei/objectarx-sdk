...
//
#pragma once
#include "GcImportXml.h"
using namespace MSXML;
#include "gcpexctl.h"
#include "opmdialog.h"
#include "Gc64BitHelpers.h"

typedef CTypedPtrArray<CPtrArray, GcTcCatalogItem*> GcTcCatalogItemArray;
BOOL GcTcUiFilterShapesDialog(GcTcCatalogItem* pPackage,
...
static void *pGcadToolImpl = NULL;
...
class ATL_NO_VTABLE GcadToolImpl :
  public CComObjectRootEx<CComSingleThreadModel>,
  public IDispatchImpl<T, piid, plibid, 1, 0>,
  public IGcadTool,
  public IGcadTool2,
  public IGcadStockTool,
  public IDropTarget,
  public IPropertyUnspecified,
  public IPerPropertyDisplayImpl<GcadToolImpl<TT, T, pclsid, szToolName, szToolImage, piid, plibid> >,
  public IOPMPropertyExtension,
  public IGcadToolContextMenu,
  public IPerPropertyBrowsing,
  public IOPMPropertyDialog,
  public IGcadToolFlyoutShape
{
public:
  GcadToolImpl()
  {
    ...
    m_pToolProp = NULL;
    mcfGscad = (CLIPFORMAT)::RegisterClipboardFormat(gcedClipFormatName());
  }
  ~GcadToolImpl()
  {
  ...
  CComPtr<IGcadToolProperties> m_pToolProp;
  bool m_bFlyout;
  GUID m_FlyoutPackageId;
  GUID m_CurrentShapeId;
  WCHAR m_FlyoutTools[64];

  BEGIN_COM_MAP(GcadToolImpl)
    COM_INTERFACE_ENTRY(T)
    COM_INTERFACE_ENTRY(IDispatch)
    COM_INTERFACE_ENTRY(IGcadTool)
    COM_INTERFACE_ENTRY(IGcadStockTool)
    ...
    COM_INTERFACE_ENTRY(IGcadToolContextMenu)
    COM_INTERFACE_ENTRY(IPerPropertyBrowsing)
    COM_INTERFACE_ENTRY(IOPMPropertyDialog)
    COM_INTERFACE_ENTRY(IGcadToolFlyoutShape)
  END_COM_MAP()
...
protected:
  ...
  GcArray<TOOL_MAP_ENTRY> m_ToolMap;
  CLIPFORMAT mcfGscad;
...
public:
  STDMETHOD(CreateTool) (IUnknown** ppTool)
  {
    ...
    return _InternalQueryInterface(IID_IGcadTool, (void**)ppTool);
  }
...
public:
  ...
  {
    return _InternalQueryInterface(IID_IGcadStockTool, (void**)ppToolClass);
  }
  ...
  STDMETHOD(dropCallback)(GcDbEntity *pEntity) { return E_NOTIMPL; }
  ...
  STDMETHOD(Execute) (LONG_PTR lFlag, DWORD_PTR  hWnd, DWORD point, DWORD dwKeyState)
  {
    ...
    ::SetFocus(gdsw_gcadDocWnd());

    GcDbDatabase *pDb = gcdbHostApplicationServices()->workingDatabase();
    if (!pDb)
      return E_FAIL;
    GcAxDocLock docLock(pDb, GcAxDocLock::kNormal);
    if (docLock.lockStatus() != Gcad::eOk) {
      return E_FAIL;
    }

    GcTcTool *pFlyoutTool = NULL;
  ...
  BOOL GetFlyoutTool(GcTcTool*& pFlyoutTool)
  {
    pFlyoutTool = NULL;
    ...
    GcTcManager* pMgr = GcTcGetManager();
    pFlyoutTool = (GcTcTool*)pMgr->FindItem(flyoutToolId);
    if (pFlyoutTool == NULL)
      return FALSE;
  ...
  {
    HRESULT hr = pProp->QueryInterface(IID_IGcadToolProperties, (void**)&m_pToolProp);
    if (!m_pToolProp)
      return E_FAIL;
  ...
  {
    m_pToolProp->QueryInterface(IID_IGcadToolProperties, (void**)pToolProp);
    return S_OK;
  }
  ...
  STDMETHOD(Drop) (IDataObject __RPC_FAR *pDataObj, DWORD grfKeyState, POINTL pt, DWORD __RPC_FAR *pdwEffect)
  {
    *pdwEffect = DROPEFFECT_COPY;
    ...
    GcDbDatabase tempDb(false, true);
    Gcad::ErrorStatus es = tempDb.readDwgFile(clipInfo.szTempFile);
    if (es != Gcad::eOk)
      return E_FAIL;

    GcDbBlockTableRecordIterator *pItr = NULL;
    {
      GcDbBlockTableRecordPointer pBTR(GCDB_MODEL_SPACE, &tempDb, GcDb::kForRead);
      if (pBTR->newIterator(pItr) != Gcad::eOk)
        return E_FAIL;
    }

    GcDbEntity *pDropEntity = NULL;
    while (!pItr->done())
    {
      if (pItr->getEntity(pDropEntity, GcDb::kForRead) != Gcad::eOk)
      {
        delete pItr;
        return E_FAIL;
      }
  ...
  STDMETHOD(DoModal)(BSTR* propValueString, GcDbObjectIdArray* ObjectIdArray)
  {
    IGcadToolProperties *pToolProp = GetThisToolsProperties();
    if (!pToolProp)
      return E_FAIL;
    ...
    GcTcCatalogItem* pShapeCatalog = pMgr->GetShapeCatalog();
    GcTcCatalogItem* pShapePackage = pShapeCatalog->FindInChildren(id);
    ...
    GcTcCatalogItemArray activeShapes;
    for (long i = lLower; i <= lUpper; i++) {
      ...
      GcTcCatalogItem* pItem = pShapePackage->FindInChildren(id, FALSE);
      if (pItem == NULL)
        continue;
    ...
    GcTcTool* pCurrentShape = (GcTcTool*)pShapePackage->FindInChildren(currentShapeId, FALSE);

    if (!GcTcUiFilterShapesDialog(pShapePackage, &activeShapes, pCurrentShape, NULL))
      return E_FAIL;
    ...
    rgsaBound.cElements = GcIntPtrToInt(nCount);
    ...
    for (long i = 0; i < nCount; i++) {
      GcTcCatalogItem* pItem = activeShapes[i];
      pItem->GetID(&id);
      VARIANT varGuid;
      varGuid.vt = VT_BSTR;
  ...
  STDMETHOD(get_Color)(VARIANT *pVar)
  {
    ...
    CComQIPtr<IGcadGcCmColor> pIGcadGcCmColor;
    HRESULT hr = pIGcadGcCmColor.CoCreateInstance(CLSID_GcadGcCmColor);
    if (FAILED(hr) || !pIGcadGcCmColor)
      return E_FAIL;

    CComBSTR colorName(m_Color.colorName());
    CComBSTR bookName(m_Color.bookName());
    pIGcadGcCmColor->put_EntityColor(m_Color.color());
    pIGcadGcCmColor->SetNames(colorName, bookName);

    hr = pIGcadGcCmColor->QueryInterface(IID_IDispatch, (void**)&pVar->pdispVal);
    if (SUCCEEDED(hr))
      return S_OK;
    else
      return E_FAIL;
  ...
  STDMETHOD(put_Color)(VARIANT pVar)
  {
    ...
    HRESULT hr = S_OK;
    CComQIPtr<IGcadGcCmColor> pIGcadGcCmColor;
    hr = pVar.pdispVal->QueryInterface(IID_IGcadGcCmColor, (void**)&pIGcadGcCmColor);
    if (FAILED(hr) || !pIGcadGcCmColor)
      return E_FAIL;
    ...
    hr = pIGcadGcCmColor->get_EntityColor(&lEntityColor);
    hr = pIGcadGcCmColor->get_ColorName(&colorName);
    hr = pIGcadGcCmColor->get_BookName(&bookName);
  ...
  GcTcPalette *CreatePaletteATC(GcTcCatalog *pCatalog, LPWSTR szPaletteName)
  {
    GUID guid;

    if (!SUCCEEDED(CoCreateGuid(&guid)))
      return NULL;
    GcTcPalette *pPalette = new GcTcPalette;
    try
    {
    ...
    try
    {
      ...
      if (!pCatalog->SetDescription(szPaletteName))
        throw 0;
      if (!GcTcGetManager()->SaveCatalogs())
        throw 0;
    }
    catch (int)
    {
  ...
  GcTcTool *CreateToolATC(GcTcPackage *pPalette, LPCWSTR szToolNameOverride = NULL,
    ...
    BOOL bImageFile = FALSE)
  {
    ...
    try
    {
      ...
      GcTcImageList *pList = pTool->GetImageList();
      if (!pList)
        throw 0;
      ...
      if (!pTool->SetToolType(GcTc::kToolNormal))
        throw 0;
    ...
    try
    {
      if (!GcTcGetManager()->SaveCatalogs())
        throw 0;
      if (!GcTcGetManager()->SaveCatalogs(GcTc::kShapeCatalog))
        throw 0;
    }
    catch (int)
    {
  ...
  GcTcTool *CreateFlyoutToolATC(GcTcPackage *pPalette, GcTcPackage *pShapePackage, LPCWSTR szToolNameOverride = NULL)
  {
    GUID toolGUID;
    ...
    try
    {
      ...
      if (!pTool->SetToolType(GcTc::kToolFlyout))
        throw 0;
  ...
  GcTcTool *CreateCommandToolATC(GcTcPackage *pPalette, LPWSTR szToolName, LPCWSTR szBitmapName, LPWSTR macroString)
  {
    GUID guid;
    BOOL bResult;
  ...
  GcTcCatalog *CreateStockToolATC(LPWSTR szCatalogName)
  {
    CGcModuleResourceOverride resourceOverride;

    GUID guid;

    GcTcStockTool *pStockTool = new GcTcStockTool;
    GcTcCatalog *pStockToolCatalog = new GcTcCatalog;
    try
    {
      ...
      if (-1 == GcTcGetManager()->AddStockToolCatalog(pStockToolCatalog))
        throw 0;
    }
    catch (int)
    {
    ...
    GcTcCatalog *pCatalog = new GcTcCatalog;
    try
    {
      if (!SUCCEEDED(CoCreateGuid(&guid)))
        throw 0;
      ...
      if (-1 == GcTcGetManager()->AddCatalog(pCatalog))
        throw 0;

      if (!GcTcGetManager()->SaveCatalogs())
        throw 0;

      if (!GcTcGetManager()->SaveCatalogs(GcTc::kStockToolCatalog))
        throw 0;
    }
    catch (int)
    {
  ...
  GcTcPackage *CreateShapeCatalogATC(LPWSTR szShapeName)
  {
    GcTcManager *pManager = GcTcGetManager();
    GcTcCatalog *pShapesCatalog = NULL;
    if (pManager)
    {
      pShapesCatalog = (GcTcCatalog *)pManager->GetShapeCatalog();
      if (!pShapesCatalog)
        return NULL;
    }
    else
      return NULL;
    GcTcPackage *pPackage = GetCustomShapePackage(pShapesCatalog, szShapeName);
    if (NULL == pPackage)
    {
      pPackage = new GcTcPackage;
      try
      {
  ...
  IGcadToolProperties* GetThisToolsProperties()
  {
    GcadToolImpl<TT, T, pclsid, szToolName, szToolImage>* pThisTool = (GcadToolImpl<TT, T, pclsid, szToolName, szToolImage>*)pGcadToolImpl;

    CComQIPtr<IGcadTool> pGcadTool(pThisTool);
    if (!pGcadTool)
      return NULL;

    CComPtr<IUnknown> pUnkToolProp;
    HRESULT hr = pGcadTool->GetToolProperties(&pUnkToolProp);
    if (FAILED(hr))
      return NULL;

    CComQIPtr<IGcadToolProperties> pToolProp(pUnkToolProp);
    if (!pToolProp)
      return NULL;
  ...
  GcTcPackage* GetCustomShapePackage(GcTcCatalog *pCatalog,
    LPCWSTR szPackageName)
  {
    if (!pCatalog)
      return NULL;
    GcTcPackage *pPackage = NULL;
    for (int i = 0; i < pCatalog->GetChildCount(); i++)
    {
      pPackage = (GcTcPackage *)pCatalog->GetChild(i);
      if (pPackage)
      {
