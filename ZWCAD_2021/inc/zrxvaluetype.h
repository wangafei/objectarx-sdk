...
private:
	...
	ZCBASE_PORT ZcRxValueType(const ZcRxValueType& rhs);
	ZCBASE_PORT ZcRxValueType& operator=(const ZcRxValueType& rhs);
    
    IZcRxNonBlittableType* m_pNonBlittable;
    IZcRxEnumeration* m_pEnum;
    IZcRxReferenceType* m_pRef;
    IZcRxObjectValue* m_pRxObjValue;
    void* m_unused1; 
    unsigned int m_size;
...
inline void* operator new(size_t size, Storage* loc) { ZSOFT_UNREFED_PARAM(size); return loc;} 
#pragma pop_macro("new")

#pragma push_macro("delete")
#undef delete
inline void operator delete(void* p, Storage* loc) { ZSOFT_UNREFED_PARAM(p); (loc); } 
