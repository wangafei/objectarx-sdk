...
int zcedGetFullString(int cronly, const ZTCHAR *pString, ZTCHAR *&pResult);

template<size_t nBufLen> inline 
int zcedGetEnv(const wchar_t* pszName, wchar_t (&buf)[nBufLen])
{
	return ::zcedGetEnv(pszName, buf, nBufLen);
}

template<size_t nBufLen> inline 
int zcedGetInput(wchar_t (&buf)[nBufLen])
{
	return ::zcedGetInput(buf, nBufLen);
}

template<size_t nBufLen> inline 
int zcedGetString(int cronly, const wchar_t* prompt, wchar_t (&buf)[nBufLen])
{
	return ::zcedGetString(cronly, prompt, buf, nBufLen);
}

template<size_t nBufLen> inline 
int zcedFindFile(const wchar_t* pszName, wchar_t (&buf)[nBufLen])
{
	return ::zcedFindFile(pszName, buf, nBufLen);
}

template<size_t nBufLen> inline 
int zcedGetKword(const wchar_t* pszPrompt, wchar_t (&buf)[nBufLen])
{
	return ::zcedGetKword(pszPrompt, buf, nBufLen);
}

template<size_t nPreBufLen, size_t nNameBufLen, size_t nExtBufLen> inline 
int	zcedFNSplit(const wchar_t* pathToSplit,
