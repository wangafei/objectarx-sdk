...
class ZCDB_PORT ZcDbAssocArrayActionBody : public ZcDbAssocParamBasedActionBody
{
public:
    ...
    virtual ~ZcDbAssocArrayActionBody(void);
    ...
    virtual Zcad::ErrorStatus addMoreObjectsToDeepCloneOverride(ZcDbIdMapping& idMap,
                   ZcDbObjectIdArray& additionalObjectsToClone) const ZSOFT_OVERRIDE;

    virtual Zcad::ErrorStatus postProcessAfterDeepCloneOverride(
                                          ZcDbIdMapping& idMap) ZSOFT_OVERRIDE;

    virtual Zcad::ErrorStatus dragStatusOverride(const ZcDb::DragStat status) ZSOFT_OVERRIDE;
};
